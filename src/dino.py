...
def pick_dino_model(min_gb_7b=config.VRAM_GB_FOR_7B, verbose=True):
    ...
    if verbose:
        print(
            f"GPU: {p.name} ({gb:.0f} GB VRAM) -> {model} "
...
def setup_activity(
    ...
    inp = Input.model_validate(
        {
            "bbox": out_fgb,
            "dino_model": dino_model,
            "high_res": high_res,
            "rasters": [
                {"bands": ["RED", "GREEN", "BLUE"], "raster_file": webmap_path}
            ],
        }
    )
    act = Dinov3Embedding(inp, "", S3Mock(working_dir="/"))
    _trust_torch_load()  # PyTorch 2.6 weights_only fix (legacy .tar 7B)
    model, device = _load_model_lowmem(
        act
    )  # CPU-load -> cast -> GPU (avoids the activity OOM)
...
def _load_model_lowmem(act):
    ...
    local_file = asyncio.run(
        download_dino_model_async(name)
    )  # already on disk -> returns path
    state_dict = torch.load(
        local_file, map_location="cpu"
    )  # 26.8 GB in CPU RAM, NOT on the GPU
    model = get_dino_model(
        name, device=torch.device("cpu")
    )  # build the architecture on CPU
    ...
    if config.DINO_DTYPE == "bf16":
        ...
        print(
            "dtype: bfloat16 (weights ~half VRAM, activations halved; outputs cast back to fp32)"
        )
...
def embed_cell(act, model, device, bbox, webmap_path):
    ...
    with rasterio.open(webmap_path) as r:
        rgb = r.read(
            (1, 2, 3),
            window=from_bounds(*bbox, transform=r.transform),
            boundless=True,
            fill_value=0,
        ).transpose(1, 2, 0)
    emb = act.create_embedding(
        model, device, rgb
    )  # upscales by upsample, embeds (FP32)
    tf = tf_from_bounds(
        *bbox, emb.shape[2], emb.shape[1]
    )  # embedding geotransform for this cell
    return rgb, emb, tf
...
def embed_cell_tokens(act, model, device, bbox, webmap_path, upsample=None):
    ...
    x = (
        make_transform(rgb.shape[0] * up)(Image.fromarray(rgb))
        .unsqueeze(0)
        .to(device=device, dtype=mdtype)
    )
    ...
    pt = (
        f["x_norm_patchtokens"][0].float().cpu().numpy()
    )  # (gh*gw, C) -> fp32 for storage
