...
warnings.filterwarnings(
    "ignore"
)  # silence rio-tiler NoOverviewWarning etc. across the pipeline
...
SITE_DATA_ROOT = os.environ.get(
    "DINO_SITE_DATA_ROOT", "/mnt/spatial/DeepThought/SiteData"
)  # shared store
DATASET_VERSION = os.environ.get(
    "DINO_DATASET_VERSION", "v2_tytonai_rg"
)  # = config dataset_version
CONFIG_DIR = os.environ.get("DINO_CONFIG_DIR", os.path.join(_REPO_ROOT, "config"))
PIC_DIR = os.environ.get(
    "DINO_PIC_DIR", os.path.join(_REPO_ROOT, "outputs", "pictures")
)
_EMB_BASE = os.environ.get(
    "DINO_EMB_ROOT", "/mnt/ai/DeepThought/dino_embeddings"
)  # shared net store (base)
STATS_PARQUET = os.path.join(
    CONFIG_DIR, "tiles_stat_db", "site_resolution.parquet"
)  # multi-site only
...
DINO_MODEL = os.environ.get(
    "DINO_MODEL", "auto"
)  # "auto" picks 7B vs ViT-L by GPU VRAM
VRAM_GB_FOR_7B = float(
    os.environ.get("DINO_VRAM_GB_FOR_7B", "40")
)  # >= this -> dinov3_vit7b16
...
HIGH_RES = (
    os.environ.get("DINO_HIGH_RES", "1") == "1"
)  # default ON; doubles upsample (2048 -> 128x128 grid). Set DINO_HIGH_RES=0 to disable
DINO_DTYPE = os.environ.get(
    "DINO_DTYPE", "fp32"
).lower()  # "bf16" -> cast model+input to bfloat16 (weights ~half VRAM, activations halved); outputs still stored fp32
TILE_PATCHES = 1  # grid cell native size = TILE_PATCHES * patch_size
MIN_DATA_COV = (
    0.02  # drop grid cells with < this fraction of RGB data (all-black voids)
)
DINO_PATCH = 16
...
def site_id_from_key(site_key: str, res: str) -> str:
    ...
    """
    return re.sub(r"[^0-9A-Za-z]+", "_", f"{site_key}/{res}/{DATASET_VERSION}").strip(
        "_"
    )
...
def resolve_tiles(site_key: str, res: str | None = None) -> str:
    ...
    d = os.path.join(
        SITE_DATA_ROOT, site_key, "Raster", "ObjectData", res, DATASET_VERSION
    )
...
def resolve_rgb(site_key: str, res: str | None = None) -> dict:
    ...
    if site_key not in paths:
        raise KeyError(
            f"site '{site_key}' not in site_rgb_paths.json "
            f"({len(paths)} sites; e.g. {list(paths)[:3]})"
        )
    by_res = paths[site_key]
    if res is None:
        res = (
            default_res(site_key)
            if site_key in _load_json("sites_to_resolutions.json")
            else list(by_res)[0]
        )
    if res not in by_res:
        raise KeyError(
            f"resolution '{res}' not for site '{site_key}' (have {list(by_res)})"
        )
    rec = by_res[res]
    if not rec.get("rgb_path"):
        raise KeyError(
            f"site '{site_key}' @ {res} has no resolved rgb_path (source={rec.get('source')})"
        )
    return rec
