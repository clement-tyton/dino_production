...
def _npz_member_shape(zf: zipfile.ZipFile, name: str) -> tuple[int, ...]:
...
def _npz_member_array(zf: zipfile.ZipFile, name: str) -> np.ndarray:
...
def read_tile_bboxes(
    site_dir: str, splits: tuple[str, ...] = ("train", "val"), show_bar: bool = True
) -> gpd.GeoDataFrame:
    ...
    """
    files = [
        (s, f) for s in splits for f in glob.glob(os.path.join(site_dir, s, "*.npz"))
    ]
    rows, srid = [], None
    for s, f in tqdm(files, desc="read tiles", unit="tile", disable=not show_bar):
        with zipfile.ZipFile(f) as zf:  # ONE open per tile (over /mnt)
            ...
            gt = np.asarray(
                _npz_member_array(zf, "GEO_TRANSFORM"), float
            )  # tiny arrays
...
def webmap_extent(webmap_path: str, dst_crs: Any) -> gpd.GeoDataFrame:
    ...
    return gpd.GeoDataFrame(
        {"src_crs": [str(src_crs)]}, geometry=[box(*bb)], crs=dst_crs
    )


def webmap_footprint(
    webmap_path: str, dst_crs: Any, max_dim: int = 2000, min_area_px: int = 64
) -> gpd.GeoDataFrame:
    ...
    polys = [
        _shape(g)
        for g, v in features.shapes(mask.astype("uint8"), mask=mask, transform=t)
        if v == 1
    ]
...
def crop_tiles_to_webmap(
    gdf: gpd.GeoDataFrame, webmap_path: str, footprint: bool = True
    ...
    """
    ext = (
        webmap_footprint(webmap_path, gdf.crs)
        if footprint
        else webmap_extent(webmap_path, gdf.crs)
    )
    clipped = gpd.clip(gdf, ext)  # crops geometries to the polygon / rectangle
    return clipped, ext


def study_area(gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
...
def snap_bbox_to_patch(
    bounds: Any, transform: Affine, patch: int
    ...
    c1 = (
        c0 + math.ceil((math.ceil(max(cols)) - c0) / patch) * patch
    )  # extend to a multiple of patch
    r1 = r0 + math.ceil((math.ceil(max(rows)) - r0) / patch) * patch
    (x0, y0), (x1, y1) = transform * (c0, r0), transform * (c1, r1)
    return (min(x0, x1), min(y0, y1), max(x0, x1), max(y0, y1))


def cell_data_coverage(
    webmap_path: str, geoms: gpd.GeoSeries, n: int = 16
) -> np.ndarray:
    ...
    with rasterio.open(webmap_path) as r:
        for g in geoms:
            ...
            a = r.read(
                (1, 2, 3), window=win, boundless=True, fill_value=0, out_shape=(3, n, n)
            )
...
def build_tile_grid(
    ...
    2) keep cells intersecting >=1 training tile (inside the study area);
       skipped when full_extent=True — grid covers the entire webmap footprint instead.
    ...
    with rasterio.open(webmap_path) as r:
        ...
        webmap_bounds = r.bounds
    patch = config.activity_params(res, high_res)["patch_size"] * tile_patches
    src_bounds = webmap_bounds if full_extent else study_gdf.to_crs(wcrs).total_bounds
    bbox = snap_bbox_to_patch(src_bounds, gt, patch)
    grid = create_adaptive_grid(
        bbox, None, gt, wcrs, patch, patch, fixed_size=True
    )  # full square grid
    grid = (grid.set_crs(wcrs) if grid.crs is None else grid).reset_index(drop=True)
    n_full = len(grid)
    # (a) inside the study area: intersect >=1 training tile (skipped when full_extent=True)
    if full_extent:
        n_tiles = n_full
    else:
        tiles_w = tiles_gdf.to_crs(wcrs)
        hit = gpd.sjoin(
            grid, tiles_w[["geometry"]], predicate="intersects", how="inner"
        )
    ...
    cell_px = sorted(
        {int(round((g.bounds[2] - g.bounds[0]) / res)) for g in grid.geometry}
    )
    grid = grid.to_crs(study_gdf.crs)
    info = {
        "native_res_m": round(res, 4),
        "patch_px": patch,
        "cell_ground_m": round(patch * res, 1),
        "cell_sizes_px": cell_px,
        "n_cells_full": n_full,
        "after_tile_filter": n_tiles,
        "n_cells": len(grid),
        "dropped_void": n_tiles - len(grid),
    }
    return grid, info
