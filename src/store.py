...
def embed_grid(
    ...
    lock = patch_io.acquire_write_lock(
        site_dir
    )  # refuse a 2nd concurrent writer (corruption guard)
    try:
        z, refs, cls_list = None, [], []
        it = tqdm(geoms, desc=desc[:22] or "cells", unit="cell") if show_bar else geoms
        for i, geom in enumerate(it):
            _, emb, cls, _ = embed_cell_tokens(
                act, model, device, tuple(geom.bounds), webmap_path, upsample=upsample
            )  # emb = (C, gh, gw)
            grid = emb.transpose(1, 2, 0).astype(np.float16)  # (gh, gw, C)
            if z is None:
                z = patch_io.create(
                    site_dir, len(geoms), *grid.shape
                )  # shape known now
            z[i] = grid
...
def write_manifest(grid_w, refs, cls_vecs, site_id, part_dir, emb_root=config.EMB_ROOT):
    ...
    manifest.insert(
        0, "cell_id", np.arange(len(manifest))
    )  # cell_id == zarr index == cls row
    manifest["patch_ref"] = [
        patch_io.relref(r, emb_root) for r in refs
    ]  # "<site_id>/patches.zarr#i"
    manifest["cls"] = cls_vecs.tolist()  # CLS vector inline (self-contained)
    manifest.to_parquet(
        os.path.join(part_dir, "cells.parquet")
    )  # Hive-partitioned GeoParquet
    manifest[["cell_id", "patch_ref", "geometry"]].to_file(
        os.path.join(emb_root, site_id, "cells.fgb"), driver="FlatGeobuf"
    )  # QGIS view (reorders on read)
    np.save(
        os.path.join(emb_root, site_id, "cls_vecs.npy"), cls_vecs
    )  # cls_vecs[cell_id], order-stable
    return manifest
...
def write_site_meta(
    ...
    *,
    ...
    meta = {
        ...
        "embed_dim": c,
        "patch_grid": [gh, gw],
        "patch_px": ginfo.get("patch_px"),
        "native_res_m": ginfo.get("native_res_m"),
